...
export class Game extends Scene {
    ...
    checkAgainstChallenge(key: string) {
        if (this.curChallengeText.indexOf(key) > -1) {
            if (!this.activeTextObjects[key]) {
                this.activeTextObjects[key] = this.add.text(
                    512 * Math.random(),
                    384 * Math.random(),
                    key,
                    {
            ...
            if (this.keysDown.size === this.curChallengeText.length) {
                ...
                if (this.timer) {
                    scoreIncrement = getScore(
                        this.timer.getElapsedSeconds(),
                        this.curChallengeText.length
                    );
                }

                let next = getNextChallenge(
    ...
    create() {
        ...
        this.timeText = this.add
            .text(1024 - 75, 75, "Time: 5", { fontSize: 20 })
            .setOrigin(1, 0)
            .setDepth(1);

        this.curChallengeText = getNextChallenge(
            this.challengeNumber++,
            this.gameStrategy
        ).word;
